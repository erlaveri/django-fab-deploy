...
from fabric.api import abort, settings, cd, run, env, puts, local, sudo
...
class WebProject(TaskSet):
    ...
    def __init__(self, apps=None):
        if apps is None:
            apps = dict(django=Django(Nginx(), Uwsgi()))
        self.apps = apps
    ...
    @task_method
    def pip(self, commands='', restart=True):
        """
    ...
    @task_method
    def upload_pub_key(self, name='id_rsa.pub'):
        key = local('cat ~/.ssh/%s' % name, capture=True)
        run('mkdir -p ~/.ssh')
    ...
    def mini_deploy(self):
        ...
        dj = self.apps['django']
        dj.update_config(restart=False)
        dj.migrate()
        dj.collectstatic()
        dj.backend.touch()
